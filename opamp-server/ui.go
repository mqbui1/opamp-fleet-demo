...
func Start(rootDir string) {
	...
	mux.HandleFunc("/api/agents", listAgentsJSON)
	srv = &http.Server{
		Addr:    "0.0.0.0:4321",
		Handler: mux,
	}
...
func listAgentsJSON(w http.ResponseWriter, r *http.Request) {
	agents := data.AllAgents.GetAllAgentsReadonlyClone()
	result := make([]agentJSON, 0, len(agents))
	for _, agent := range agents {
		healthy := agent.Status != nil && agent.Status.Health != nil && agent.Status.Health.Healthy
		result = append(result, agentJSON{
